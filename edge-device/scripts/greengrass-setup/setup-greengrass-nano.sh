...
aws iot attach-thing-principal --region ap-northeast-1 --thing-name $THING_NAME \
    --principal $CERT_ARN
THING_GROUP_NAME=GGv2JetsonNanoGroup
...
POLICY_NAME=GGv2JetsonNanoIoTThingPolicy
...
GG_TES_ROLE_NAME=GGv2JetsonNanoTokenExchangeRole
...
GG_TES_POLICY_ARN=$(aws iam create-policy --policy-name GGv2JetsonNanoTokenExchangeRoleAccess --policy-document file://device-role-access-policy.json | jq -r '.Policy.Arn')
aws iam attach-role-policy --role-name $GG_TES_ROLE_NAME --policy-arn $GG_TES_POLICY_ARN
GG_TES_ROLE_ALIAS_ARN=$(aws iot create-role-alias --region ap-northeast-1 --role-alias GGv2JetsonNanoTokenExchangeRoleAlias --role-arn $GG_TES_ROLE_ARN | jq -r '.roleAliasArn')
GG_TES_POLICY_NAME=GGv2JetsonNanoTokenExchangeRoleAliasPolicy
...
sed -i.bak "s|<ggv2_thing_name>|$THING_NAME|" $certs_folderconfig.yaml && \
 sed -i.bak "s|<ggv2_role_alias>|GGv2JetsonNanoTokenExchangeRoleAlias|" $certs_folderconfig.yaml && \
 sed -i.bak "s|<iot_data_endpoint>|$IOT_DATA_ENDPOINT|" $certs_folderconfig.yaml && \
 sed -i.bak "s|<iot_cred_endpoint>|$IOT_CRED_ENDPOINT|" $certs_folderconfig.yaml