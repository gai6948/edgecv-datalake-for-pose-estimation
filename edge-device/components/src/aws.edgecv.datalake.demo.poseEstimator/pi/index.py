...
s3_prefix = "pose-estimator-demo/processed-video-frames/pi/"
...
def dst_gstreamer_pipeline(
    ...
    return (
        ...
        'x264enc bframes=0 key-int-max=45 bitrate=500 ! h264parse ! '
...
class VideoStream:
    ...
    def __init__(self, resolution=(640, 480), framerate=30):
        ...
        self.stream = cv2.VideoCapture(src_gstreamer_pipeline(
            resolution[0], resolution[1], framerate), cv2.CAP_GSTREAMER)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # template_show = np.squeeze(frame_resized / 255)
            # template_show = np.array(template_show*255,np.uint8)
            # template_kps = parse_output(target_heatmaps,target_offsets,0.6)
            # out_img = draw_kps(template_show.copy(),template_kps)
            ...
            if producer_timestamp > current_timestamp:
                ...
                img_filename = 'pi-pose-output-' + str(producer_timestamp) + '.jpg'
